...
# Function to auto-install missing dependencies
install_dependencies() {
    print_status "WARN" "Attempting automatic installation of missing tools..."
    
    if command -v apt-get &> /dev/null; then
        print_status "INFO" "Detected Debian/Ubuntu (APT)."
        sudo apt-get update
        sudo apt-get install -y qemu-system qemu-utils cloud-image-utils wget
    elif command -v dnf &> /dev/null; then
        print_status "INFO" "Detected Fedora/RHEL/Alma/Rocky (DNF)."
        sudo dnf install -y qemu-kvm qemu-img cloud-image-utils wget
    elif command -v yum &> /dev/null; then
        print_status "INFO" "Detected older CentOS/RHEL (YUM)."
        sudo yum install -y qemu-kvm qemu-img cloud-image-utils wget
    elif command -v pacman &> /dev/null; then
        print_status "INFO" "Detected Arch Linux (Pacman)."
        sudo pacman -Sy --noconfirm qemu-full cloud-image-utils wget
    elif command -v zypper &> /dev/null; then
        print_status "INFO" "Detected openSUSE (Zypper)."
        sudo zypper install -y qemu-x86 qemu-tools cloud-image-utils wget
    else
        print_status "ERROR" "No supported package manager found (apt, dnf, yum, pacman, zypper)."
        print_status "ERROR" "Please install dependencies manually."
        exit 1
    fi
}
...
check_dependencies() {
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        # Re-verify after attempted installation
        local still_missing=()
        for dep in "${deps[@]}"; do
            if ! command -v "$dep" &> /dev/null; then
                still_missing+=("$dep")
        ...
        if [ ${#still_missing[@]} -ne 0 ]; then
            print_status "ERROR" "Installation failed. Still missing: ${still_missing[*]}"
            exit 1
        else
            print_status "SUCCESS" "All dependencies installed and verified!"
        fi
    fi
}
...
# Check dependencies (now auto-installs via package manager)
check_dependencies
