...
run_menu() {
    while true; do
        ...
        echo -e " ${DG}──────────────────────────────────────────────────────────┐${NC}"
        read -rp " ${Y}Select an option [1-8]:${NC} " choice
        case "$choice" in
            1) start_antiddos ;;
            2) clear_rules ;;
            3) status_antiddos ;;
            ...
            5)
                ...
                read -rp " ${Y}Choice [1-3]:${NC} " wl_choice
                case "$wl_choice" in
                    1) manage_ipset "whitelist" "list" "" ;;
                    2) read -rp " ${Y}Enter IP to whitelist:${NC} " ip; manage_ipset "whitelist" "add" "$ip" ;;
                    3) read -rp " ${Y}Enter IP to remove:${NC} " ip; manage_ipset "whitelist" "del" "$ip" ;;
                    *) echo -e " ${R}✗ Invalid option!${NC}" ;;
                esac
                ;;
            6)
                echo -e "\n ${BOLD}Blacklist Operations:${NC} 1) List  2) Add  3) Remove"
                read -rp " ${Y}Choice [1-3]:${NC} " bl_choice
                case "$bl_choice" in
                    1) manage_ipset "blacklist" "list" "" ;;
                    2) read -rp " ${Y}Enter IP to blacklist:${NC} " ip; manage_ipset "blacklist" "add" "$ip" ;;
                    3) read -rp " ${Y}Enter IP to remove:${NC} " ip; manage_ipset "blacklist" "del" "$ip" ;;
                    *) echo -e " ${R}✗ Invalid option!${NC}" ;;
                esac
                ;;
            7) scan_system ;;
        ...
        echo ""
        read -rp " ${DG}Press Enter to return to menu...${NC}"
    done
}
